"""
main module for building a neural network
has layers, basic network and helper functions
"""
...
from .activations import Activation, activation_functions as af
from . import loss
from . import metrics
...
def train_test_split(
    ...
    shuffle_data: boolean
        whether we should shuffle the data first
...
class BatchNorm(Layer):
    ...
    def __init__(
        ...
        # first moment for Adam/momentum
        self.m_gamma = np.zeros_like(self.beta)
        ...
        # set needed attributes as None for now
    ...
    def update_weights(
        ...
        if optimizer == "sgd":
            ...
            self.v_gamma = (
                beta2 * self.v_gamma
                + (1 - beta2) * (self.grad_gamma ** 2)
            )
            self.v_beta = (
                beta2 * self.v_beta
                + (1 - beta2) * (self.grad_beta ** 2)
            )
            ...
            self.gamma -= (
                learning_rate * m_hat_gamma
                / (np.sqrt(v_hat_gamma) + epsilon)
            )
            self.beta -= (
                learning_rate * m_hat_beta
                / (np.sqrt(v_hat_beta) + epsilon)
            )
    ...
    def __init__(self,
        ...
        if isinstance(activation, str):
            ...
            if act_name in activations.activation_functions:
                ...
                self.activation: Activation = af[act_name](**activation_params)
            else:
                keys = list(af.keys())
                raise ValueError(
        ...
        if weights is not None:
            ...
            if self.activation_name in ["relu", "leaky_relu", "elu", "swish"]:
                # He initialization
                self.W = np.random.randn(self.n, self.m) * np.sqrt(2. / self.m)
            elif self.activation_name in ["sigmoid", "tanh"]:
                # Xavier/Glorot initialization
                self.W = np.random.randn(self.n, self.m) * np.sqrt(1. / self.m)
            else:
                # Slightly larger fallback
                self.W = np.random.randn(self.n, self.m) * 0.01
...
class FeedForward:
    ...
    def __init__(
        ...
        for i, layer in enumerate(layers):
            if (
                getattr(layer, "activation_name", "identity") == "softmax"
            ...
            if getattr(layer, 'has_dims', False):
                if last_dim_layer is not None:
                    if layer.m != last_dim_layer.n:
                        err = (
                            f"Mismatched dimensions: layer {last_index}"
                            f"has {last_dim_layer.n} outputs, "
    ...
    def backward(self, grad_output: np.ndarray):
        """
        backward method for the network
        """
    ...
    def set_training(self, mode: bool = True):
        """
        set the training method for all layers
        """
        self.training = mode
        for layer in self.layers:
    ...
    def fit(
        ...
        val_set: Optional[
            tuple[
                Union[np.ndarray, List, DataFrame],
                Union[np.ndarray, List, DataFrame, Series]
                ]
            ] = None,
        ...
        loss_fn : str or loss.Loss
            ...
            If a string, must be a key in `loss.loss_functions`.
        verbose : bool
        ...
        val_size : int or float
            ...
            If a float in (0,1), treated as a fraction of the dataset;
            otherwise, treated as the number of samples.
        ...
        if optimizer not in ["sgd", "momentum", "adam"]:
            raise ValueError(
                f"{optimizer} is not a valid optimizer. Choose one of sgd, momentum, or adam"
                )
        ...
        for epoch in range(1, epochs+1):
            ...
            for batch_idx in range(num_batches):
                ...
                if verbose:
                    ...
                    if batch_idx % print_every == 0:
                        ...
                        print(
                            f"Epoch {epoch}/{epochs}, Batch {batch_idx+1}/{num_batches}, "
                            f"Loss: {running_ave_loss:.4f}"
                        )
            # at the end of the epoch print out the epoch loss/metric
            train_metric_values = {
                name: func(self.predict(X_train), y_train) for name, func in metric_dict.items()
                }
            ...
            train_metrics_str = ", ".join(
                f"{name}: {val:.4f}" for name, val in train_metric_values.items()
                )
            ...
            # we compute the loss/accuracy on the validation set from that epoch and report it
            else:
                y_test_pred = self.predict(X_val)
                val_metric_values = {
                    name: func(y_test_pred, y_val) for name, func in metric_dict.items()
                    }
                ...
                if lambda_l1 > 0 or lambda_l2 > 0:
                    ...
                    for layer in self.layers:
                        for attr in layer.regularizable_params:
                            ...
                            if lambda_l2 > 0:
                                val_reg_loss += (
                                    0.5 * lambda_l2 * np.sum(param ** 2) / X_val.shape[0]
                                )
                    val_loss += val_reg_loss
                history["val_loss"].append((epoch+last_epoch, val_loss))
                val_metrics_str = ", ".join(
                    f"{name}: {val:.4f}" for name, val in val_metric_values.items()
                    )
    ...
    def predict(self, X: Union[DataFrame, np.ndarray, List]):
        ...
        np.ndarray, shape (n_samples, 1) or (n_samples, n_outputs)
            ...
            For models with a single output neuron, returns a 2D array with shape (n_samples, 1).
...
def evaluate(model: FeedForward, X: Union[DataFrame, np.ndarray, List]):
    """
    wrapper to turn the model into just evaluating
    """
